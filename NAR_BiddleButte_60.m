...
%trains on 2012 60 minute data, tests on 2013 60 minute data.
clear all; close all; 
...
lag = 10; %number of 60 min samples lag
graphend = lag+1;
%develop narnetwork with time delay of "lag" 60 minute samples
...
RMSEtrain = sqrt(perform(net,Ts,Y)) %mean square error
...
title( 'BIDDLE BUTTE NARNET RESULTS, 60 MINUTE DATA, 2012' );
...
BB_y_2013 = BB_y_2013 - repmat(m,size(BB_y_2013,1),1);
T_2013 = num2cell(BB_y_2013)';
%BiddleButte_Data = BiddleButte_Data(:,1);
D_2013 = num2cell(BB_x_2013)';
...
axis tight; %set axis
legend( 'TARGET', 'OUTPUT' );
title( 'BIDDLE BUTTE NARNET RESULTS, 60 MINUTE DATA, 2013' );
%%
figure(3);
hold on;
...
axis tight;
legend('TARGET','OUTPUT');
title('BIDDLE BUTTE DETAIL, 60 MINUTE DATA, 2013');